...
import { MultisigAccountRow } from '../pageElements/AccountRow.js';
...
export class MultisigAccountsPage extends Page {
  constructor () {
    super(
  ...
  async getAccountRows (): Promise<MultisigAccountRow[]> {
    const table = await this.getTable();
    const rows = await table.getRows();

    return rows.map((row) => new MultisigAccountRow(row.primaryRow, row.detailsRow));
  }
