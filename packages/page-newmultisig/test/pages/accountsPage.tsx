...
import { NewMultisigAccountRow } from '../pageElements/AccountRow.js';
...
export class NewMultisigAccountsPage extends Page {
  constructor () {
    super(
  ...
  async getAccountRows (): Promise<NewMultisigAccountRow[]> {
    const table = await this.getTable();
    const rows = await table.getRows();

    return rows.map((row) => new NewMultisigAccountRow(row.primaryRow, row.detailsRow));
  }
